...
codes_after = [{"code":"4591","system":"TADDS"},{"code":"4942","system":"TADDS"}];
MIN_DAYS = 31;
MAX_DAYS = 186;

with open(sys.argv[1], 'r') as file_in, open('long-covid-potential-cases.csv', 'w', newline='') as file_out:
    csv_reader = csv.DictReader(file_in)
    csv_writer = csv.DictWriter(file_out, csv_reader.fieldnames + ["long-covid-symptoms-muscle---tadds-31-to-186-days-after-covid19-identified"])
    csv_writer.writeheader();
    codes_identified = 0;
    for row in csv_reader:
        new_row = row.copy();
        new_row["long-covid-symptoms-muscle---tadds-31-to-186-days-after-covid19-identified"] = "UNK";
        ...
        if(len(matching_before_codes)>0 and len(matching_after_codes)>0): 
            if(MIN_DAYS < (matching_after_codes[0][1]-matching_before_codes[0][1]).days and (matching_after_codes[0][1]-matching_before_codes[0][1]).days < MAX_DAYS):
                new_row["long-covid-symptoms-muscle---tadds-31-to-186-days-after-covid19-identified"] = "CASE";
            else:
                new_row["long-covid-symptoms-muscle---tadds-31-to-186-days-after-covid19-identified"] = "UNK";
